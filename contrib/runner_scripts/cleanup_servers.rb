...
Account.find(:all, :conditions => ["cloud_servers_username IS NOT NULL AND cloud_servers_username != '' and cloud_servers_api_key IS NOT NULL and cloud_servers_api_key != ''"], :group => "cloud_servers_api_key").each do |acct|
	...
	begin
	CloudServersUtil.cloud_server_init(acct.cloud_servers_username, acct.cloud_servers_api_key)
	CloudServersUtil.all_servers do |cs|
			...
			if server.nil? then
				...
				begin
					...
					Timeout::timeout(30) do
						CloudServersUtil.destroy_server(cs.id)
					end
				rescue
				end
