...
cloud_servers_vpc[:auth_username]= username that will be required to access
 the web UI and REST interfaces.

cloud_servers_vpc[:auth_password]= password that will be required to access
 the web UI and REST interfaces.

cloud_servers_vpc[:rackspace_cloud_username]= Rackspace Cloud account username

cloud_servers_vpc[:rackspace_cloud_api_key]= Rackspace Cloud account api key
